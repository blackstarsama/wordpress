...
if( !function_exists( 'cpis_import' ) )
{
	function cpis_import()
	{
		...
		if( isset( $_REQUEST[ 'cpis_total_products_processed' ] ) ) $cpis_total_products_processed = @intval( $_REQUEST[ 'cpis_total_products_processed' ] );
		...
		{
			throw new Exception( __( 'The URL to the XML file is required', CPIS_TEXT_DOMAIN ) );
		}

		$cpis_xml_url = sanitize_text_field( $_REQUEST[ 'cpis_xml_url' ] );
		...
		if( !file_exists( $path ) )
		{
			throw new Exception( __( 'The XML file is not accessible', CPIS_TEXT_DOMAIN ) );
		}

		register_shutdown_function('cpis_shutdown');
		$dir  = dirname( $path );

		if( ( $xmlObj = simplexml_load_file( $path ) ) === false )
		{
			throw new Exception( __( 'The file structure is wrong', CPIS_TEXT_DOMAIN ) );
		}

		if( !property_exists( $xmlObj, 'product' ) )
		{
			throw new Exception( __( 'The products to import are required', CPIS_TEXT_DOMAIN ) );
		}
		...
		{
			$counter++;
			if( $counter < $cpis_total_products_processed ) continue;
			...
			if( property_exists( $p, 'thumbnail' ) )
			{
				...
				if(
					...
					{
						throw new Exception( __( 'The intermediate image has not been generated', CPIS_TEXT_DOMAIN ) );
					}
				}
				else
				{
					throw new Exception( __( 'The thumbnail image has not been generated', CPIS_TEXT_DOMAIN ) );
				}
			}
			...
			if( property_exists( $p, 'image' ) )
			{
				...
				$images_dir = date( 'Y/m' );
				if(
					!( ( $uploads = wp_upload_dir( $images_dir ) ) && false === $uploads['error'] )
				)
				{
					throw new Exception( __( 'Has not been possible create the subfolders in the "Uploads" directory for storing the images', CPIS_TEXT_DOMAIN ) );
				}
				...
				foreach( $p->image as $image )
				{
					...
					{
						throw new Exception( __( 'The images does not exists', CPIS_TEXT_DOMAIN ).': '.$original_path );
					}

					$wp_filetype = wp_check_filetype( $original_path, null );
					if ( !$wp_filetype[ 'type' ] || !$wp_filetype[ 'ext' ] )
					{
						throw new Exception( __( 'The file type is not supported', CPIS_TEXT_DOMAIN ).': '.$original_path );
					}
					...
					{
						throw new Exception( __( 'The file cannot be copied to the final location', CPIS_TEXT_DOMAIN ).': '.$uploads[ 'path' ] );
					}
		...
		throw new Exception( '<div style="text-align:center;"><h1>'.__( 'All Images Imported', CPIS_TEXT_DOMAIN ).'</h1></div>' );
	}
} // End cpis_import
...
if( !function_exists( 'cpis_shutdown' ) )
{
	function cpis_shutdown()
	{
		...
		if( empty( $cpis_recursive_call ) ) return;
?>
		<form id="cpis_shutdown" action="<?php print get_admin_url( get_current_blog_id() ); ?>" method="post">
			<input type="hidden" name="cpis-action" value="import" />
			<input type="hidden" name="cpis_total_products_processed" value="<?php print $cpis_total_products_processed; ?>" />
			<input type="hidden" name="cpis_xml_url" value="<?php print esc_attr( sanitize_text_field($_REQUEST[ 'cpis_xml_url' ]) ); ?>" />
