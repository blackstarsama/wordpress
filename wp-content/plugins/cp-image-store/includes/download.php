				...
				if( count( $urls ) ){
					if( empty( $download_links_str ) ){
						foreach($urls as $url){
							...
							if( $download_link !== $url->link ) $download_link = CPIS_H_URL.'?cpis-action=f-download'.( ( isset( $GLOBALS[CPIS_SESSION_NAME][ 'cpis_user_email' ] ) ) ? '&cpis_user_email='.$GLOBALS[CPIS_SESSION_NAME][ 'cpis_user_email' ] : '' ).'&f='.$download_link.( ( !empty( $_REQUEST[ 'purchase_id' ] ) ) ?  '&purchase_id='.sanitize_key($_REQUEST[ 'purchase_id' ]) : '' );
