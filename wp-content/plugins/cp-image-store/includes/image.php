...
if( !function_exists( 'cpis_image_metabox' ) ){
    function cpis_image_metabox(){
        ...
        $query = $wpdb->prepare("SELECT * FROM ".$wpdb->prefix.CPIS_IMAGE." as data WHERE data.id = %d;", $post->ID);
        $data = $wpdb->get_row($query);

        $query = $wpdb->prepare("SELECT file.* FROM ".$wpdb->prefix.CPIS_FILE." as file, ".$wpdb->prefix.CPIS_IMAGE_FILE." as image_file WHERE file.id=image_file.id_file AND image_file.id_image=%d", $post->ID);
            ...
            <table class="form-table product-data">
                <tr>
                    ...
                    echo '
                            ...
                            <input type="text" id="new_author" placeholder="'.esc_attr(__('Enter a new author', CPIS_TEXT_DOMAIN)).'">
                             <input type="button" value="'.esc_attr(__('Add author', CPIS_TEXT_DOMAIN)).'" class="button" onclick="cpis_add_element(\'new_author\', \'cpis_author_list\', \'cpis_author_new\');"/><br />
                ...
                <tr>
                    ...
                    echo '
                             ...
                             <input type="text" id="new_type" placeholder="'.esc_attr(__('Enter a new type', CPIS_TEXT_DOMAIN)).'">
                             <input type="button" value="'.esc_attr(__('Add type', CPIS_TEXT_DOMAIN)).'" class="button" onclick="cpis_add_element(\'new_type\', \'cpis_type_list\', \'cpis_type_new\');" /><br />
                ...
                <tr>
                    ...
                    echo '
                             ...
                             <input type="text" id="new_color" placeholder="'.esc_attr(__('Enter a new color scheme', CPIS_TEXT_DOMAIN)).'">
                             <input type="button" value="'.esc_attr(__('Add color scheme', CPIS_TEXT_DOMAIN)).'" class="button" onclick="cpis_add_element(\'new_color\', \'cpis_color_list\', \'cpis_color_new\');" /><br />
        ...
        echo '
            <input type="button" value="'.esc_attr(__('Add File', CPIS_TEXT_DOMAIN)).'" onclick="cpis_new_file();" />
                ...
                echo '
                    <tr>
                        <td style="width:55%;">
                            ...
                            <input type="hidden" name="cpis_file['.$file->id.']" value="'.esc_attr($file->id).'" />
                        </td>
                        ...
                        <td style="width:10%;"> <input type="button" onclick="cpis_remove_file(this, '.esc_js($file->id).');" value="'.esc_attr(__( 'Remove', CPIS_TEXT_DOMAIN )).'" /></td>
                    </tr>
                 ';
            }
        }
        ...
        echo '
                <tr>
                    ...
                    <td style="width:10%;"> <input type="button" onclick="cpis_remove_file(this);" value="'.esc_attr(__( 'Remove', CPIS_TEXT_DOMAIN )).'" /></td>
                </tr>
            </table>
        ';
    }
...
if( !function_exists( 'cpis_save_image' ) ){
    function cpis_save_image($post_id, $post, $update){
        ...
        if(isset($_POST['cpis_author'])){
			if(!is_array($_POST['cpis_author'])) $_POST['cpis_author'] = array($_POST['cpis_author']);
			$_POST['cpis_author'] = array_map('sanitize_text_field', $_POST['cpis_author']);
            wp_set_object_terms($id, $_POST['cpis_author'], 'cpis_author', true);
        }

        if(isset($_POST['cpis_author_new'])){
			if(!is_array($_POST['cpis_author_new'])) $_POST['cpis_author_new'] = array($_POST['cpis_author_new']);
			$_POST['cpis_author_new'] = array_map('sanitize_text_field', $_POST['cpis_author_new']);
            wp_set_object_terms($id, $_POST['cpis_author_new'], 'cpis_author', true);
        }

        // Set the type list
        if(isset($_POST['cpis_type'])){
			if(!is_array($_POST['cpis_type'])) $_POST['cpis_type'] = array($_POST['cpis_type']);
			$_POST['cpis_type'] = array_map('sanitize_text_field', $_POST['cpis_type']);
            wp_set_object_terms($id, $_POST['cpis_type'], 'cpis_type', true);
        }

        if(isset($_POST['cpis_type_new'])){
            if(!is_array($_POST['cpis_type_new'])) $_POST['cpis_type_new'] = array($_POST['cpis_type_new']);
			$_POST['cpis_type_new'] = array_map('sanitize_text_field', $_POST['cpis_type_new']);
            wp_set_object_terms($id, $_POST['cpis_type_new'], 'cpis_type', true);
        }

        // Set the color scheme list
        if(isset($_POST['cpis_color'])){
            if(!is_array($_POST['cpis_color'])) $_POST['cpis_color'] = array($_POST['cpis_color']);
			$_POST['cpis_color'] = array_map('sanitize_text_field', $_POST['cpis_color']);
            wp_set_object_terms($id, $_POST['cpis_color'], 'cpis_color', true);
        }

        if(isset($_POST['cpis_color_new'])){
            if(!is_array($_POST['cpis_color_new'])) $_POST['cpis_color_new'] = array($_POST['cpis_color_new']);
			$_POST['cpis_color_new'] = array_map('sanitize_text_field', $_POST['cpis_color_new']);
        ...
        if( isset( $_POST[ 'cpis_file' ] ) ){
            foreach( $_POST[ 'cpis_file' ] as $key => $value ){
                $wpdb->update(
                    ...
                    array(
                        'width'  => @floatval($_POST[ 'cpis_file_width' ][ $key ]),
                        'height' => @floatval($_POST[ 'cpis_file_height' ][ $key ]),
                        'price'  => @floatval($_POST[ 'cpis_file_price' ][ $key ])
                    ),
                ...
                if( empty( $movefile[ 'error' ] ) ){
                    $wpdb->insert(
                        ...
                        array(
                            'url'    => $movefile[ 'url' ],
                            'path'   => $movefile[ 'file' ],
                            'width'  => @floatval($_POST[ 'cpis_file_width_new' ][ $key ]),
                            'height' => @floatval($_POST[ 'cpis_file_height_new' ][ $key ]),
                            'price'  => @floatval($_POST[ 'cpis_file_price_new' ][ $key ])
                        ),
                        array(
                            '%s',
                            '%s',
    ...
    function cpis_display_content( $id, $type = 'multiple', $output = 'echo' ){
        ...
        if( !is_null( $image ) ){
            ...
            if( !empty( $files ) ){
                ...
                $data_arr[ 'button' ] = '<input type="button" id="cpis-paypal-button" class="cpis-paypal-button" value="'.esc_attr(__( 'Buy Now', CPIS_TEXT_DOMAIN )).'" onclick="cpis_buynow(this);" />';
        ...
        switch( $type ){
            ...
            case 'single':
				...
				if( !empty($options[ 'store' ][ 'facebook_app_id' ])) $data_arr[ 'facebook_app_id' ] = trim($options[ 'store' ][ 'facebook_app_id' ]);

                if( !empty( $options[ 'store' ][ 'store_url' ] ) ) $data_arr[ 'store_url' ] = cpis_complete_url($options[ 'store' ][ 'store_url' ]);
 ...
 if( !function_exists( 'cpis_print_single' ) ){
    function cpis_print_single( $arr ){
        ...
        $code .= '
            ...
            ( ( isset( $arr[ 'store_url' ] ) )  ? '<div class="cpis-store-page-link"><a href="'.esc_url($arr[ 'store_url' ]).'">'.__( 'Return to the store page', CPIS_TEXT_DOMAIN ).'</a></div>' : '' ).'
