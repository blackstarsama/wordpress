...
if(!function_exists('cpis_start_session'))
{
	function cpis_start_session()
	{
		...
		{
			$GLOBALS['CPIS_SESSION_ID'] = $_COOKIE[CPIS_SESSION_NAME];
		...
		if($set_cookie)
		{
			...
			if(!headers_sent()) @setcookie( CPIS_SESSION_NAME, $GLOBALS['CPIS_SESSION_ID'], 0, '/' );
		}
	}
	cpis_start_session();
}
...
if(!function_exists('cpis_redirect_to_settings'))
{
	function cpis_redirect_to_settings($plugin, $network_activation)
	{
		...
		{
			exit( wp_redirect( admin_url( 'admin.php?page=image-store-menu-settings' ) ) );
		}
	}
}
...
if( !function_exists( 'cpis_create_db' ) ){
    function cpis_create_db( $installing = false ){
        try{
            if( !$installing && !empty( $GLOBALS[CPIS_SESSION_NAME][ 'cpis_created_db' ] ) ) return;
        ...
        if( !is_admin() ){
            ...
            if( isset( $_REQUEST ) && isset( $_REQUEST[ 'cpis-action' ] ) ){
                ...
                switch( strtolower( $_REQUEST[ 'cpis-action' ] ) ){
					...
					default:
						if(stripos($_REQUEST['cpis-action'],'ipn|') !== false)
						{
							if(!empty($options['store']['debug_payment']))
							{
								@error_log('Image Store payment gateway GET parameters: '.json_encode($_GET));
								@error_log('Image Store payment gateway POST parameters: '.json_encode($_POST));
							}
...
if( !function_exists( 'cpis_display_post_states' ) )
{
	...
	{
		if($post->post_name == 'cpis-download-page')
			$post_states['cpis-download-page'] = 	'Image Store - '.__('Download Page', CPIS_TEXT_DOMAIN);
...
if( !function_exists( 'cpis_admin_init' ) ){
    ...
    function _cpis_create_pages( $slug, $title ){
		if( isset( $GLOBALS[CPIS_SESSION_NAME][ $slug ] ) ) return $GLOBALS[CPIS_SESSION_NAME][ $slug ];
    ...
    function cpis_admin_init(){
		...
		if( isset( $_REQUEST[ 'cpis-action' ] ) ){
			...
			elseif( $_REQUEST[ 'cpis-action' ] == 'import' )
			{
				...
				try
				{
					if( !(wp_verify_nonce( $_POST['cpis_import'], 'session_id_'.session_id() ) &&  current_user_can( 'manage_options' ) ) )
					{
						throw new Exception( __( 'You have not sufficient privileges to import images', CPIS_TEXT_DOMAIN ) );
					}
				...
				catch( Exception $err )
				{
					...
					print '<div style="text-align:center;"><h1>'.$err->getMessage().'</h1></div>';
				}
				exit;
			}
            ...
            switch( strtolower( $_REQUEST[ 'cpis-action' ] ) ){
                case 'remove-image':
                    if( !empty( $_REQUEST[ 'image' ] ) ){
                        print cpis_remove_image( @intval($_REQUEST[ 'image' ]) );
                    }else{
...
if( !function_exists( 'cpis_customAdjustmentsLink' ) ){
    function cpis_customAdjustmentsLink( $links ){
        ...
        $help_link = '<a href="https://wordpress.org/support/plugin/cp-image-store#new-post" target="_blank">'.__( 'Help', CPIS_TEXT_DOMAIN ).'</a>';
...
if(!function_exists('cpis_preview'))
{
	function cpis_preview()
	{
		...
		if(array_intersect($allowed_roles, $user->roles ))
		{
			...
			{
				// Sanitizing variable
				$preview = sanitize_text_field(stripcslashes($_REQUEST['cpis-preview']));
				...
				)
				{
					...
					$if_empty = __('There are no products that satisfy the block\'s settings', CPIS_TEXT_DOMAIN);
					...
					print '<div class="cpis-preview-container">'.$output.'</div>';
...
if( !function_exists( 'cpis_store_button' ) ){
    function cpis_store_button(){
        ...
        if( isset( $post ) && $post->post_type != 'cpis_image' ){
            print '<a href="javascript:cpis_insert_store();" title="'.esc_attr(__( 'Insert Image Store', CPIS_TEXT_DOMAIN )).'"><img src="'.esc_url(CPIS_PLUGIN_URL.'/images/image-store-icon.png').'" alt="'.esc_attr(__( 'Insert Image Store', CPIS_TEXT_DOMAIN )).'" /></a>';

            print '<a href="javascript:cpis_insert_product_window();" title="'.esc_attr(__( 'Insert Image Product', CPIS_TEXT_DOMAIN )).'"><img src="'.esc_url(CPIS_PLUGIN_URL.'/images/image-store-insert-product.png').'" alt="'.esc_attr(__( 'Insert Image Product', CPIS_TEXT_DOMAIN )).'" /></a>';
        }
    }
...
if( !function_exists( 'cpis_menu_links' ) ){
    function cpis_menu_links(){
        if(is_admin()){
            ...
            // Importing Submenu
            add_submenu_page(CPIS_IMAGE_STORE_SLUG, __('Importing Area', CPIS_TEXT_DOMAIN), __('Importing Area', CPIS_TEXT_DOMAIN), 'manage_options', CPIS_IMAGE_STORE_SLUG.'-import', 'cpis_import_page');
 ...
 if( !function_exists( 'cpis_settings_page' ) ){
    function cpis_settings_page(){
        ...
        if (isset($_POST['cpis_settings']) && wp_verify_nonce( $_POST['cpis_settings'], plugin_basename( __FILE__ ) ) ){
            ...
            $cpis_currency_symbol   = sanitize_text_field( $_POST['cpis_currency_symbol'] );
            $cpis_currency          = sanitize_text_field( $_POST['cpis_currency'] );
            $cpis_language          = sanitize_text_field( $_POST['cpis_language'] );

            if( !empty( $_POST[ 'cpis_layout' ] ) )
            {
				$_POST['cpis_layout'] = sanitize_text_field($_POST['cpis_layout']);
            ...
            $noptions['paypal'] = array(
                ...
                'paypal_email'      => sanitize_text_field($_POST['cpis_paypal_email']),
                'currency_symbol'   => ( ( !empty( $cpis_currency_symbol ) ) ? sanitize_text_field($_POST['cpis_currency_symbol']) : '$' ),
                'currency'          => ( ( !empty( $cpis_currency ) ) ? sanitize_text_field($_POST['cpis_currency']) : 'USD' ),
                'language'          => ( ( !empty( $cpis_language ) ) ? sanitize_text_field($_POST['cpis_language']) : 'Eng' ),
                'tax'     			=> ( ( !empty( $_POST['cpis_tax'] )  && ($cpis_tax = sanitize_text_field($_POST['cpis_tax'])) != '') ? @floatval($cpis_tax) : '' ),
                'shopping_cart'     => false
            );

            $thumbnail_w = sanitize_text_field( $_POST[ 'cpis_thumbnail_width' ] );
            $thumbnail_h = sanitize_text_field( $_POST[ 'cpis_thumbnail_height' ] );

            $intermediate_w = sanitize_text_field( $_POST[ 'cpis_intermediate_width' ] );
            $intermediate_h = sanitize_text_field( $_POST[ 'cpis_intermediate_height' ] );

            $noptions['image'] = array(
                'unit'              => sanitize_text_field($_POST['cpis_unit']),
                'set_watermark'     => false,
                'watermark_text'    => 'Image Store',
                'license'           => array(
                                            'title'       => sanitize_text_field($_POST['cpis_license_title']),
                                            'description' => wp_kses_post($_POST['cpis_license_description'])
                                       ),
                'thumbnail'         => array(
            ...
            $noptions['display'] = array(
                'carousel' => array(
                    ...
                    'transition_time'   => sanitize_text_field($_POST['cpis_carousel_transition_time']),
                ),
                'preview' => ( ( isset( $_POST['cpis_activate_preview'] ) ) ? true : false )
            );

            $noptions['store'] = array(
                'store_url'          => esc_url_raw($_POST['cpis_store_url']),
                ...
                'items_page'         => sanitize_text_field($_POST['cpis_items_page']),
                'social_buttons'     => ( ( isset( $_POST['cpis_social_buttons'] ) ) ? true : false ),
				'facebook_app_id'	 => ( ( isset( $_POST['cpis_facebook_app_id'] ) ) ? sanitize_text_field($_POST['cpis_facebook_app_id']) : '' ),
                'columns'            => sanitize_text_field($_POST['cpis_columns']),
                'pack_files'         => false,
                'download_link'      => sanitize_text_field($_POST['cpis_download_link']),
                'download_limit'     => sanitize_text_field($_POST['cpis_download_limit']),
            ...
            $noptions['notification'] = array(
                'from'                  => sanitize_text_field($_POST['cpis_from']),
                'to'                    => sanitize_text_field($_POST['cpis_to']),
                'subject_payer'         => sanitize_text_field($_POST['cpis_subject_payer']),
                'subject_seller'        => sanitize_text_field($_POST['cpis_subject_seller']),
                'notification_payer'    => wp_kses_data(stripcslashes($_POST['cpis_notification_payer'])),
                'notification_seller'   => wp_kses_data(stripcslashes($_POST['cpis_notification_seller']))
            );
...
<?php
        ...
        $options = get_option( 'cpis_options' );
		if($options['notification']['from'] == 'put_your@emailhere.com')
		{
			...
			if ($pos === false) $options['notification']['from'] = 'admin@'.$domain;
		}

		if($options['notification']['to'] == 'put_your@emailhere.com')
		{
			if(!isset($user_email)) $user_email = get_the_author_meta('user_email', get_current_user_id());
			if(!empty($user_email)) $options['notification']['to'] = $user_email;
		}
?>
        ...
        <form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
            ...
            <div class="postbox">
                ...
                <div class="inside">
                    <table class="form-table">
                        ...
                        <tr valign="top">
                            ...
                            <td>
                                ...
                                <?php
                                    ...
                                    {
                                        print '<option value="'.esc_attr($id).'" '.( ( !empty( $cpis_layout ) && $id == $cpis_layout[ 'id' ] ) ? 'SELECTED' : '' ).' thumbnail="'.esc_url($layout[ 'thumbnail' ]).'">'.$layout[ 'title' ].'</option>';
                                    }
                                ?>
                                </select>
						...
						<tr valign="top">
							<th><?php _e('Prevent the products pages be cached', CPIS_TEXT_DOMAIN); ?></th>
							<td><input type="checkbox" name="cpis_prevent_cache" value="1" <?php if (get_option('cpis_prevent_cache', true)) echo 'checked'; ?> /></td>
						</tr>
						<tr valign="top">
                            <th><?php _e('Show buttons for sharing in social networks', CPIS_TEXT_DOMAIN); ?></th>
                            <td>
                                <input type="checkbox" name="cpis_social_buttons" <?php echo ( ( $options[ 'store' ][ 'social_buttons' ] ) ? 'CHECKED' : '' ); ?> /> <em><?php _e('The option enables the buttons for sharing the products in social networks', CPIS_TEXT_DOMAIN); ?></em>
                            </td>
                        </tr>
                        <tr valign="top">
							<th><?php _e('Facebook app id for sharing in Facebook', CPIS_TEXT_DOMAIN); ?></th>
							<td>
								...
								<em><?php _e('Click the link to generate the Facebook App and get its ID: <a target="_blank" href="https://developers.facebook.com/apps">https://developers.facebook.com/apps</a>', CPIS_TEXT_DOMAIN); ?></em>
							</td>
						</tr>
                    </table>
                </div>
			...
			<div class="postbox">
                <div class="inside">
					<table class="form-table">
						<tr>
							<th>
								<?php _e('Restrict the access to registered users only', CPIS_TEXT_DOMAIN); ?>
							</th>
							<td>
								<input type="checkbox" name="cpis_download_link_for_registered_only" id="cpis_download_link_for_registered_only" <?php echo(!empty($options[ 'store' ][ 'download_link_for_registered_only' ]) ? 'CHECKED' : ''); ?> /> <?php _e('Display the free download links only for registered users', CPIS_TEXT_DOMAIN)?>
							</td>
						</tr>
					</table>
				</div>
            ...
            <div class="postbox">
                ...
                <div class="inside">
					<table class="form-table">
                        ...
                        <tr valign="top">
                        <th scope="row"><?php _e( 'Apply taxes (in percentage)', CPIS_TEXT_DOMAIN ); ?></th>
                        <td><input type="text" name="cpis_tax" value="<?php if(!empty($options [ 'paypal' ][ 'tax' ])) echo esc_attr( $options [ 'paypal' ][ 'tax' ] ); ?>" /></td>
                        </tr>
                        ...
                        <tr valign="top">
                        <th scope="row"><?php _e( 'Pack all purchased files as a single ZIP file', CPIS_TEXT_DOMAIN ); ?></th>
                        <td><input type="checkbox" disabled >
                        <?php
						...
						<tr>
							<td colspan="2">
								...
								<div style="border:1px solid #ddd;padding:15px;">
									<input type="checkbox" name="cpis_debug_payment" <?php if(!empty($options[ 'store' ][ 'debug_payment' ])) print 'CHECKED'; ?> /> <b><?php _e('Debugging Payment Process', CPIS_TEXT_DOMAIN); ?></b><br /><br />
									<i><?php _e("(If the checkbox is ticked the plugin will create two new entries in the error  logs file on your server, with the texts <b>Image Store payment gateway GET parameters</b> and <b>Image Store payment gateway POST parameters</b>.  If after a purchase, none of these entries appear in the error logs file, the payment notification has not reached the plugin's code)", CPIS_TEXT_DOMAIN); ?></i>
								</div>
							</td>
						</tr>
                     </table>
            ...
            <div class="postbox">
                ...
                <div class="inside">
                    ...
                    <div><input type="checkbox" disabled /> <?php _e('Display discount promotions in the image store page', CPIS_TEXT_DOMAIN)?></div>
                    ...
                    <table class="form-table cpis_discount_table" style="border:1px dotted #dfdfdf;">
                        <tr>
                            ...
                            <td style="font-weight:bold;"><?php _e('In Sales over than ... ', CPIS_TEXT_DOMAIN); echo($options[ 'paypal' ][ 'currency' ]); ?></td>
                    ...
                    <table class="form-table">
                        ...
                        <tr valign="top">
                            ...
                            <td><input type="text" disabled /> <?php echo $options[ 'paypal' ][ 'currency' ]; ?></td>
                        </tr>
                        <tr valign="top">
			...
			?>
            <div class="submit"><input type="submit" class="button-primary" value="<?php esc_attr_e(__('Update Settings', CPIS_TEXT_DOMAIN)); ?>" /></div>
            </form>
<?php
    }
 ...
 if( !function_exists( 'cpis_import_page' ) ){
	...
	?>
		<form method="post" action="<?php echo esc_url($_SERVER[ 'REQUEST_URI' ]); ?>" id="import_form" target="_blank" >
			<input type="hidden" name="cpis-action" value="import" />
			<!-- NOTIFICATIONS BOX -->
            <div class="postbox">
                <h3 class='hndle' style="padding:5px;"><span><?php _e('Importing Area', CPIS_TEXT_DOMAIN); ?></span></h3>
                <div class="inside">
                    <table class="form-table">
                        <tr valign="top">
                        <th scope="row"><?php _e('URL to XML file', CPIS_TEXT_DOMAIN); ?></th>
                        <td>
							<input type="text" name="cpis_xml_url" size="40" value="" />
							<input type="submit" value="<?php _e( 'Import', CPIS_TEXT_DOMAIN ); ?>" class="button-primary" />
						</td>
                        </tr>
                    </table>
					<div>
...
?>
        <form method="post" action="<?php echo esc_url($_SERVER[ 'REQUEST_URI' ]); ?>" id="purchase_form">
        ...
        <div class="postbox" style="margin-top:20px;">
            ...
            <div class="inside">
				<div>
					...
					<input type="text" name="from_year" value="<?php esc_attr_e($from_year); ?>" size="5" />
					...
					<input type="submit" value="<?php esc_attr_e(__('Search', CPIS_TEXT_DOMAIN)); ?>" class="button-primary" />
					<input type="button" value="<?php esc_attr_e(__('Export CSV', CPIS_TEXT_DOMAIN)); ?>" class="button" onclick="cpis_export_csv( this );" />
				</div>
            ...
            <div class="inside">
				<?php
					...
					if(count($purchase_list)){
						print '
							<div>
								<label style="margin-right: 20px;" ><input type="checkbox" onclick="cpis_load_report(this, \'sales_by_country\', \''.esc_js(__( 'Sales by country', CPIS_TEXT_DOMAIN )).'\', \'residence_country\', \'Pie\', \'residence_country\', \'count\');" /> '.esc_js(__( 'Sales by country', CPIS_TEXT_DOMAIN )).'</label>
								<label style="margin-right: 20px;" ><input type="checkbox" onclick="cpis_load_report(this, \'sales_by_currency\', \''.esc_js(__( 'Sales by currency', CPIS_TEXT_DOMAIN )).'\', \'mc_currency\', \'Bar\', \'mc_currency\', \'sum\');" /> '.__( 'Sales by currency', CPIS_TEXT_DOMAIN ).'</label>
								<label><input type="checkbox" onclick="cpis_load_report(this, \'sales_by_product\', \''.esc_js(__( 'Sales by product', CPIS_TEXT_DOMAIN )).'\', \'product_name\', \'Bar\', \'post_title\', \'sum\');" /> '.__( 'Sales by product', CPIS_TEXT_DOMAIN ).'</label>
							</div>';
					}
				?>
                        ...
                        foreach( $purchase_list as $purchase ){
                            if( $group_by == 'no_group' )
							{
								...
								if( $to_display == 'sales' )
								{
									...
									echo '
										<TR>
											<TD>'.$purchase->date.'</TD>
											<TD><a href="'.esc_url(get_permalink($purchase->ID)).'" target="_blank">'.$purchase->post_title.'</a></TD>
							...
							else
							{
								...
								if( $to_display == 'sales' ){
									echo '
											<TR>
												<TD><a href="'.esc_url(get_term_link($purchase->term_slug, $group_by )).'" target="_blank">'.$purchase->term_name.'</a></TD>
												<TD>'.(round( $purchase->purchase_total*100)/100).'</TD>
											</TR>
										';
								}elseif(  $to_display == 'amount'  ){
...
<?php
 ...
 if( !function_exists( 'cpis_admin_enqueue_scripts' ) ){
    function cpis_admin_enqueue_scripts( $hook ){
        ...
        if(
            ...
            wp_enqueue_style('jquery-style', '//ajax.googleapis.com/ajax/libs/jqueryui/1.8.3/themes/smoothness/jquery-ui.css');
 ...
 if( !function_exists( 'cpis_enqueue_scripts' ) ){
    function cpis_enqueue_scripts(){
        if(
            ...
            wp_enqueue_script('cpis-script', CPIS_PLUGIN_URL.'/js/public.js', array('jquery', 'json2', 'jquery-ui-core', 'cpis-carousel', 'jquery-ui-position'), null);
			...
			if( !empty( $cpis_layout ) )
			{
				...
				if( !empty( $cpis_layout[ 'script_file' ] ) ) wp_enqueue_script('cpis-js-layout', $cpis_layout[ 'script_file'] , array( 'cpis-script' ), false);
			}
 ...
 if( !function_exists( 'cpis_upload_dir' ) ){
    function cpis_upload_dir( $path ){
        ...
        if(
			...
			(
				isset( $_REQUEST[ 'cpis-action' ] ) && $_REQUEST[ 'cpis-action' ] == 'import' &&
				wp_verify_nonce( $_POST['cpis_import'], 'session_id_'.session_id() ) &&
                ...
                switch( strtolower( $_REQUEST[ 'cpis-action' ] ) ){
                    case 'download':
						...
						if( empty( $cpis_errors ) ){
							...
							$error = ( !empty( $_REQUEST[ 'error_mssg' ] ) ) ? wp_kses_post($_REQUEST[ 'error_mssg' ]) : '';
 ...
 if( !function_exists( 'cpis_replace_shortcode' ) ){
    ...
    function cpis_replace_shortcode( $atts, $content, $tag ){
        ...
        $page_id = 'cpis_page_'.get_the_ID();
        if( !isset( $GLOBALS[CPIS_SESSION_NAME][ $page_id ] ) ) $GLOBALS[CPIS_SESSION_NAME][ $page_id ] = array();
        ...
        if( !isset( $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_page' ] ) ) $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_page' ] = 0;
        ...
        if( !isset( $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_search_terms' ] ) && !empty( $atts[ 'search' ] ) ) $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_search_terms' ] = $atts[ 'search' ];
        if( !isset( $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_type' ] ) && !empty( $atts[ 'type' ] ) ) $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_type' ] = $atts[ 'type' ];
        if( !isset( $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_category' ] ) && !empty( $atts[ 'category' ] ) ) $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_category' ] = $atts[ 'category' ];
        if( !isset( $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_author' ] ) && !empty( $atts[ 'author' ] ) ) $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_author' ] = $atts[ 'author' ];
        if( !isset( $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_color' ] ) && !empty( $atts[ 'color' ] ) ) $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_color' ] = $atts[ 'color' ];
        if( !isset( $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_ordering' ] ) )
        {
            if( !empty( $atts[ 'orderby' ] ) ) $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_ordering' ] = $atts[ 'orderby' ];
            else $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_ordering' ] = "post_title";
        }

        // Extract search terms
        if( isset( $_REQUEST[ 'search_terms' ] ) ){
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_search_terms' ] = sanitize_text_field($_REQUEST[ 'search_terms' ]);
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_page' ] = 0;
            $filter = _cpis_create_search_filter( sanitize_text_field($_REQUEST[ 'search_terms' ]) );
        }else{
            $filter = "";
        }
        ...
        if( isset( $_REQUEST[ 'filter_by_type' ] ) ){
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_type' ] = sanitize_text_field($_REQUEST[ 'filter_by_type' ]);
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_page' ] = 0;
        }

        if( isset( $_REQUEST[ 'filter_by_category' ] ) ){
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_category' ] = sanitize_text_field($_REQUEST[ 'filter_by_category' ]);
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_page' ] = 0;
        }

        if( isset( $_REQUEST[ 'filter_by_author' ] ) ){
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_author' ] = sanitize_text_field($_REQUEST[ 'filter_by_author' ]);
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_page' ] = 0;
        }

        if( isset( $_REQUEST[ 'filter_by_color' ] ) ){
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_color' ] = sanitize_text_field($_REQUEST[ 'filter_by_color' ]);
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_page' ] = 0;
        }

        if( isset( $_REQUEST[ 'ordering_by' ] ) ){
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_ordering' ] = sanitize_text_field($_REQUEST[ 'ordering_by' ]);
            $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_page' ] = 0;
        }
        ...
        $_order_by 	= "ORDER BY ".( ( $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_ordering'] != 'purchases' ) ? "posts" : "posts_data" ).".".$GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_ordering']." ".( ( $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_ordering'] == 'post_title' ) ? "ASC" : "DESC" );
        $_limit 	= "";

        if( ( !empty( $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_type'] )     && $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_type'] != -1 )   ||
            ( !empty( $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_color'] )    && $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_color'] != -1 )  ||
            ( !empty( $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_author'] )   && $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_author'] != -1 ) ||
            ( !empty( $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_category'] ) && $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_category'] != -1 )
        ){
            ...
            if( !empty( $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_author'] ) && $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_author' ] != -1 ){
            ...
            if( !empty( $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_color'] ) && $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_color' ] != -1 ){
            ...
            if( !empty( $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_category'] ) && $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_category' ] != -1 ){
        ...
        if( $options[ 'store' ][ 'show_pagination' ] && is_numeric( $options[ 'store' ][ 'items_page' ] ) &&  $options[ 'store' ][ 'items_page' ] > 1 ){
            ...
            $page_href = '?'.((!empty($_SERVER['QUERY_STRING'])) ? preg_replace('/(&)?cpis_page=\d+/', '', $_SERVER['QUERY_STRING']).'&' : '');
        ...
        if( $_show_search_box ||
            ...
            if( $_show_search_box ){
                ...
                <div class='cpis-filter'>
                    <input type='search' name='search_terms' placeholder='".esc_attr(__( 'Search...', CPIS_TEXT_DOMAIN ))."' value='".esc_attr( ( isset( $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_search_terms' ] ) ) ? $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_search_terms' ] : '' )."' style='width:100%;' />
                </div>
                ";
            }
                ...
                $header .= "
                        ...
                        "
                                    <select id='ordering_by' name='ordering_by' onchange='this.form.submit();'>
                                        <option value='post_title' ".( ( $GLOBALS[CPIS_SESSION_NAME][ $page_id ][ 'cpis_ordering' ] == 'post_title') ? "SELECTED" : "").">".__( 'Title', CPIS_TEXT_DOMAIN )."</option>
                                        <option value='purchases' ".( ( $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_ordering'] == 'purchases' ) ? "SELECTED" : "" ).">".__( 'Popularity', CPIS_TEXT_DOMAIN )."</option>
                                        <option value='post_date' ".( ( $GLOBALS[CPIS_SESSION_NAME][ $page_id ]['cpis_ordering'] == 'post_date') ? "SELECTED" : "" ).">".__( 'Date', CPIS_TEXT_DOMAIN )."</option>
                                    </select>
                                </form>
                            </div>
                        ";
 ...
 if( !function_exists( 'cpis_getIP' ) )
{
 ...
 if( !function_exists( 'cpis_check_download_permissions' ) ){
	function cpis_check_download_permissions(){
		...
		{
			_e( 'The purchase has not being registered yet or the ID is incorrect, please, try again in 30 minutes.', CPIS_TEXT_DOMAIN );
			return false;
		}
		...
		if( isset( $_REQUEST[ 'f' ] ) )
		{
			$_REQUEST[ 'f' ] = sanitize_text_field($_REQUEST[ 'f' ]);
		...
		if(	!empty( $GLOBALS[CPIS_SESSION_NAME][ 'cpis_download_for_free' ] ) || current_user_can( 'manage_options' ) ) return true;
		...
		if( get_option( 'cpis_safe_download', CPIS_SAFE_DOWNLOAD ) ){
			if( !empty( $_REQUEST[ 'cpis_user_email' ] ) ) $GLOBALS[CPIS_SESSION_NAME][ 'cpis_user_email' ] =  sanitize_text_field($_REQUEST[ 'cpis_user_email' ]);
			...
			$data = $wpdb->get_row( $wpdb->prepare( 'SELECT CASE WHEN checking_date IS NULL THEN DATEDIFF(NOW(), date) ELSE DATEDIFF(NOW(), checking_date) END AS days, downloads, id FROM '.$wpdb->prefix.CPIS_PURCHASE.' WHERE purchase_id=%s AND email=%s ORDER BY checking_date DESC, date DESC', array( sanitize_key($_REQUEST[ 'purchase_id' ]), $GLOBALS[CPIS_SESSION_NAME][ 'cpis_user_email' ] ) ) );
		}else{
			$data = $wpdb->get_row( $wpdb->prepare( 'SELECT CASE WHEN checking_date IS NULL THEN DATEDIFF(NOW(), date) ELSE DATEDIFF(NOW(), checking_date) END AS days, downloads, id FROM '.$wpdb->prefix.CPIS_PURCHASE.' WHERE purchase_id=%s ORDER BY checking_date DESC, date DESC', array( sanitize_key($_REQUEST[ 'purchase_id' ]) ) ) );
		}
		...
		if( is_null( $data ) ){
			if( !isset( $_REQUEST[ 'timeout' ] ) )
            {
                cpis_setError(
                    ...
                    <script>
                        var timeout_text = "'.__( 'The store should be processing the purchase. You will be redirected in', CPIS_TEXT_DOMAIN ).'";
                    </script>'
                );
            }
            else
		...
		if( isset( $_REQUEST[ 'f' ] ) && !isset( $GLOBALS[CPIS_SESSION_NAME][ 'cpis_donwloads' ] ) )
		{
...
if( !function_exists( 'cpis_download_file' ) ){
	function cpis_download_file(){
		...
		if( isset( $_REQUEST[ 'f' ] ) && cpis_check_download_permissions() ){
			$file = basename( sanitize_text_field($_REQUEST[ 'f' ]) );
			try
			{
 ...
 */
 class CPISProductWidget extends WP_Widget {
    ...
    function form( $instance ) {
        ...
        ?>
            <p><label for="<?php esc_attr_e($this->get_field_id('title')); ?>"><?php _e('Title:'); ?> <input class="widefat" id="<?php esc_attr_e($this->get_field_id('title')); ?>" name="<?php esc_attr_e($this->get_field_name('title')); ?>" type="text" value="<?php esc_attr_e($title); ?>" /></label></p>

            <p>
                <label for="<?php esc_attr_e($this->get_field_id('product_id')); ?>"><?php _e('Enter the product ID:', CPIS_TEXT_DOMAIN); ?><br />
                    <input class="widefat" id="<?php esc_attr_e($this->get_field_id( 'product_id' )); ?>" name="<?php esc_attr_e($this->get_field_name( 'product_id' )); ?>" value="<?php esc_attr_e($product_id); ?>" />
                </label>
            </p>
        <?php
    }
